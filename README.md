...
- **Manual UX**: press **→** (or click the panel button) to advance one bar. **Shift+→** (or "Skip day") fast-forwards past the rest of the session day, flattening any open position at that day's close and still raising the mandatory post-trade review for it — for boring days, or when you only trade the first hour. Buy / Sell open a mandatory pre-trade journal (setup tag, thesis, conviction, size, target, notes). Every closed trade triggers a mandatory post-trade review modal — sessions are not dismissable without journaling.
