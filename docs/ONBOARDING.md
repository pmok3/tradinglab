...
- **Menu bar** — `File` (Load/Save Configuration, Load/Save Watchlists, Theme…), `Indicators` (add indicators per scope, save/load presets, clear all), `Sandbox` (bar-replay sessions, Download Replay Data…, save/load, performance review), `Strategies` (Entries: New/Manage/Disarm All; Exits: Edit; Strategy Tester…), `View` (Heikin-Ashi submenu with candle + flat-bar highlight toggles, Highlight Key Bars, Volume time-of-day shading, ChartStack), `Tools` (Configure Credentials…, Export Bars to CSV…, Status History…, Reveal Data Folder, Restore Default Templates…), `Help` (About, Getting Started…, Keyboard Shortcuts…, a Guides submenu with the per-topic guides + Documentation Library + View Online Docs, Reveal Data Folder, Check for Updates…, Export Diagnostic Bundle…, Reset & Quit (purge data folder)…).
