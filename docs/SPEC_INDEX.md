...
| `gui/color_palette.spec.md` | Thin wrapper around the native OS color chooser (`tkinter.colorchooser.askcolor`); used by indicator legend "Change Color…" and the per-output swatches in `IndicatorDialog`. Custom HSV / honeycomb palette retired — audit `color-picker-native-only`. |
