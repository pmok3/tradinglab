...
| `strategy_tester/interval_compat.spec.md` | `incompatible_indicators_for_interval(entry, exit, interval)` (tester, normalized) + `incompatible_arming_problems(entry, *, available_intervals, fallback_interval)` (live + sandbox arming, per-condition interval) — flag intraday-only indicators (VWAP, RVOL cumulative/time-of-day, RRVOL, Prior Day H/L) on a daily/weekly interval, or (sandbox) condition trees needing finer bars than the session serves. Back the GUI run-block + arm-block popups. Audit `intraday-interval-guard`. |
...
| `strategy_tester/export.spec.md` | `export_html` (single-file HTML with relative screenshot links) + `export_pdf` (matplotlib `PdfPages`: cover + breakouts + equity curve + one landscape page per screenshot). Pure read-only; both default to writing inside the Run dir as `report.html` / `report.pdf`. |
...
| `gui/color_palette.spec.md` | Themed Win-ChooseColor look-alike (`ThemedColorChooser` + `pick_color`); used by indicator legend "Change Color…" and the per-output swatches in `IndicatorDialog`. Chrome follows app light/dark theme; persistent 16-slot custom colours. Audit `themed-color-chooser`. |
...
| `gui/_modal_base.spec.md` | `BaseModalDialog` (geometry, modal keys, opt-in `_fit_form_width`) + `make_scrollable_form(parent, *, horizontal, bind_mousewheel) -> (inner_frame, canvas)` and `protect_combobox_wheel(root, scroll_target=)`. Width fitting measures an explicitly supplied inner form/viewport, not a chart/table's natural size. |
| `gui/flow_layout.spec.md` | Opt-in measured wrapping of existing compact action rows, preserving widget identity and explicit visibility. |
...
| `gui/quant_app.spec.md` | `QuantAppMixin` — `ChartApp` glue for the Quant tab: hidden-at-startup lifecycle, the View → Quant checkbutton, watchlist-style primary/compare double-click routing, and the visibility-gated daily Last refresh through the shared `_apply_watchlist_snapshot_from_bars` seam. |
