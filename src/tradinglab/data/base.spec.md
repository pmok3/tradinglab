...
- **Return `None` OR empty list for failure**: both mean "give up, move on to fallback". Callers check truthiness (`if not candles: ...`).
...
- `tests/unit/data/test_range_fetch.py` pins the range-capability contract: `supports_range` flag round-trip, `fetch_range` status taxonomy (`ok`/`empty`/`unsupported`/`error`), datetime conversion, plain-reregister clears the flag, and the Alpaca explicit-range path.
