...
- **Index aliases share that ONE wrapper** rather than adding a second layer. `_ratio_aware` resolves `index_aliases.resolve_symbol(ticker, source_name)` FIRST, so ratio legs are already in vendor form when fetched and a scale constant is never mistaken for a symbol. A separate `_alias_aware` layer was rejected because it would break the documented `DATA_SOURCES[n].__wrapped__ is fetcher` invariant (pinned by `test_ratio_source.py` and `test_user_visible_sources.py`). Sources with no alias column (alpaca, local, synthetic) pass symbols through untouched.
