...
- `register_source(name, fetcher, *, internal=False, supports_range=False)` — imperative registration. Pass `internal=True` to keep the source out of every user-facing dropdown; pass `supports_range=True` for fetchers that accept kw-only `start` / `end`.
