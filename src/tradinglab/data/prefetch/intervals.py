...
* the escape hatch next — a daily (``1d``) chart's escape is the drilldown
...
drilldown). Pure (no Tk / IO); unit-tested in
...
#: drilldown -> 5m). Always included in the returned order.
...
def dual_interval(active_interval: str | None) -> list[str]:
    ...
    if not active:
        return ["1d", "5m"]
    order = [active]
    # Daily on-screen -> the drilldown target (5m) is the escape hatch; any
