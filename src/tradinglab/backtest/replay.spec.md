...
- `register_ticker(symbol, candles, *, prefetch_events=True) -> List[Any]` — add a symbol mid-session; returns the per-symbol *visible* candle list (stable identity for the session). `prefetch_events=False` skips the per-symbol events fetch and is what the bulk market-feed warm ([sandbox_feed](sandbox_feed.spec.md)) uses — hundreds of event round-trips for glyphs nobody is looking at is not a warm, it's a stall. Registration is otherwise identical; the symbol is fully tradeable and clock-synced.
...
- `set_focus(symbol)` — swap primary chart; honours `display_interval`. Also tops up the symbol's event bundle when it was registered with `prefetch_events=False`, guarded by `_events_prefetch_requested` so a symbol with no events doesn't re-fetch on every focus flip.
...
- **Day fast-forward (`skip_to_next_day`)**: ergonomic escape hatch for a boring day, or a trader who only works the open — one action instead of ~78 `next_bar` presses. Deliberately NOT a playback clock: the replay stays untimed (unlimited deliberation per bar is a design goal, not an omission), so this only collapses bars the user has already decided not to trade. Four steps:
  1. Tick to the LAST bar of the current session day with `_defer_render` set. That flag suppresses the matplotlib redraw, `panel.refresh()`, **and the display-only watchlist hook**; engine ticks, visible-list growth, `_invalidate_focused`, the post-trade review flow, and the scanner / entries / exits hooks still run per bar, so every side effect of a skip is identical to holding down "next bar". `_pending_day_changed` accumulates the day-rollover flag across the suppressed ticks. The watchlist is caught up once in step 2 — it is a repaint, and repainting it ~78 times for a table nobody sees between the first and last bar is pure cost. Anything that can *fire* (scanner alerts feeding `SCANNER_ALERT` entries, entries, exits) is deliberately NOT coalesced.
  2. Render once via `_install_focus_for_display` (the same chokepoint `set_focus` / `cycle_to_next` use), then run `_refresh_watchlist_for_sandbox` once for the whole batch.
...
- **Per-tick app callbacks** (`next_bar`): after `panel.refresh()`, invokes optional Tk hooks via `getattr(self.app, name, None)`:
  - `_refresh_watchlist_for_sandbox()` — **skipped while `_defer_render` is set** (display-only; see the fast-forward section).
  - `_refresh_scanner_for_sandbox()` — runs every saved scan against `visible_candles_by_symbol`; see [`app.spec.md`](../app.spec.md) §"Scanner tab integration". Consumer-gated in [sandbox_app](sandbox_app.spec.md) because the market feed makes this universe large.
