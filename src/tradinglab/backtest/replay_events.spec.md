...
## Design Decisions
- **Explicit boundary, not a transitive import**: the engine never depends on `events`; the controller does the translation once at install time. Headless Strategy Tester runs skip events entirely without touching engine code.
