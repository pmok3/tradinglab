...
- `register_corporate_actions(symbol, bundle)` — register a per-symbol `EventBundle` so dividends / splits are applied automatically during the corporate-action phase. Engine-version-stable (`ENGINE_VERSION="sandbox-1d"` unchanged). Idempotent on same-content re-register.

## Dependencies
- Internal: [`bars`](bars.spec.md), [`clock`](clock.spec.md), [`fills`](fills.spec.md), [`journal`](journal.spec.md), [`orders`](orders.spec.md), [`portfolio`](portfolio.spec.md), [`session`](session.spec.md).
- External: `numpy`.
...
- **Three-phase tick** in fixed order: (1) fills → (2) MAE/MFE roll → (3) mark-to-market. A fill on this tick must contribute to MAE/MFE *from* this bar; opening the cursor before phase 2 makes that automatic.
