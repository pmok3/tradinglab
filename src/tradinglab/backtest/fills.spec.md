...
## Public API
- `apply_fills(orders, next_bar_opens, next_bar_ts, slippage_bps, commission) -> List[Fill]` — build fills for every order whose symbol has a next-bar open in `next_bar_opens`. Orders for absent symbols are silently skipped (the engine re-queues them).
...
- No commission by default. No spread. Slippage = 0 by default.
- Fill price is `open[t+1]` exactly when slippage is zero.
- P/L is best-case; do not interpret backtest equity as net-of-costs.

## Design Decisions
- **Pure function, no shared state**. Slippage / commission live on the engine's `SessionSpec` and are passed in per call; the function itself has no globals. Required for the determinism contract.
...
## Invariants
- For a fixed `(orders, next_bar_opens, next_bar_ts, slippage_bps, commission)` tuple the output `Fill` list is byte-identical across calls (Q-12 reproducibility commitment).
