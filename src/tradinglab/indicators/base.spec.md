...
- **`output_key_label(key) -> str` classmethod.** Hook for the compact per-output band label shown beside each value in the in-chart readout legend (`gui/readout_legend.py:_key_label_for`). Default returns the raw `key` (e.g. Bollinger's `"upper"` / `"lower"`). Override to shorten verbose **canonical** output keys for display WITHOUT renaming the persisted `style` / per-output-visibility key — e.g. Prior Day H/L/C maps `prior_day_high → pd_high`. Keeping the canonical key stable means colour overrides + visibility flags still round-trip on disk.
