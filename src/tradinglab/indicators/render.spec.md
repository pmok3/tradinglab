...
- **Clickable in-pane labels** — the upper-left pane-name `Text` is created with `picker=True` and carries `_sc_pane_label_config_ids` plus `_sc_pane_label_scope`. `InteractionMixin` hit-tests that artist so lower-pane indicators open the same per-indicator popup / context menu path as overlay legend rows.
