...
- `LIGHT_THEME: dict`, `DARK_THEME: dict` — palettes keyed by named slot (`win_bg`, `fig_bg`, `ax_bg`, `text`, `text_disabled`, `grid`, `spine`, `tree_bg`, `tree_fg`, `bull_row_bg`/`fg`, `bear_row_bg`/`fg`, `tooltip_bg`/`fg`, `watermark`, `pre_shade`, `post_shade`, `crosshair`). The `text_disabled` slot is the foreground for disabled menu entries (e.g. empty recent-file placeholders); GitHub-muted greys (`#8b949e` light, `#6e7681` dark) avoid the Windows-default etched/embossed disabled-text style that looks blurry against dark backgrounds. `gui.menu_theme` consumes `win_bg`/`text`/`grid`/`text_disabled` to force classic `tk.Menu` widgets (including cascade arrows) onto the dark palette. Consumed by `app._apply_theme`, `rendering.style_axes`, `rendering.draw_session_shading`, `gui/interaction.py` overlays, and `gui/theme_controller._apply_menubar_theme`.
