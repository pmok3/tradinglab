...
- `chart_equity_to_candle(decoded: Mapping, *, tz=timezone.utc) -> Optional[Candle]` — returns `None` if any required OHLCV/timestamp field is missing.
...
- `chart_equity_to_candle` never raises — partial messages return `None`.
