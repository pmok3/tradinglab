...
- `apply_loaded_config(parent_widget)` — re-applies a freshly-loaded settings file to live app state: timezone, scroll-zoom direction, theme overrides, startup defaults, **the active light/dark theme** (sets `parent_widget.dark_var` from the resolved `startup_defaults['theme']`, then cascades `parent_widget._apply_theme()` — audit `config-theme-roundtrip`), **the saved watchlist (notebook) width** (via `parent_widget._apply_notebook_width_setting()` — audit `watchlist-width-setting`), and **the persisted live view/behaviour settings** (Heikin-Ashi, key-bar / HA-flat highlights, time-of-day volume, colour-blind palette, drawing snap, ChartStack visibility, UI scale, worker pool — via `parent_widget._apply_persisted_view_settings()`, audit `config-roundtrip-meta`), then triggers a render, calls `settings.mark_clean()` (the value setters re-write identical values, which would otherwise mark the just-loaded store dirty), and a title update. Indicator state is **not** applied — configuration is decoupled from the indicator manager (audit `config-indicators-decoupled`); a legacy `indicators` key in a loaded file is ignored.
- `load_config(parent_widget)` / `save_config(parent_widget)` / `save_config_as(parent_widget)` — File menu handlers; uses `filedialog` + `messagebox` so the GUI thread owns the prompt. Both save paths call `_capture_layout_into_settings(parent_widget)` (which invokes `parent_widget._capture_notebook_width_setting()` **and** `parent_widget._capture_theme_setting()`) **before** exporting, so the user's dragged chart|watchlist divider position and the active light/dark theme are persisted in the saved config (audits `watchlist-width-setting`, `config-theme-roundtrip`). Indicator state is intentionally **not** captured — configuration files are decoupled from the indicator manager (audit `config-indicators-decoupled`); named presets persist on their own via `indicators.preset_store`.
...
## Design notes
- **Defensive best-effort paths** — reapply, recent-list, and title-refresh helpers swallow exceptions because they run from menu callbacks where a non-critical failure should not kill the UI. User-facing load/save failures go through helper return values or `messagebox.showerror`.
...
## Dependencies
- Internal: `..defaults`, `..recent_files`, `..settings`, `..constants` (BUILTIN_STARTUP_DEFAULTS, resolve_startup_defaults), `..data.ratio_display_label`. Late: `.._version` for the title-bar version segment.
...
## Tests
Exercised by smoke checks for `config-theme-roundtrip`, `config-roundtrip-meta`, and `config-indicators-decoupled`. Dedicated unit coverage lives in `tests/unit/gui/test_config_theme_roundtrip.py`, `test_config_roundtrip_meta.py`, `test_config_indicator_decoupling.py`, and `test_notebook_width_setting.py`.
