...
- `ExportCacheDialog(parent)` — Toplevel dialog modal to `parent`.
  Loads the cache index at construction time.
...
- **Destination chosen via `filedialog.askdirectory`**. Required before
  Export; if missing, the status row reports the error rather than
  silently failing. Status row also reports "Nothing selected" when the
  user has clicked Select None.
...
- **`export_entries` is the single bottleneck**. Per-entry failures
...
- **Empty cache → friendly message + Close-only**. Refuses to open a
  ...
  button.
- **Atomic write via `local_export.write_csv`**. The dialog itself
  doesn't touch the file system — all writes go through the same
  vetted exporter as the future programmatic export API.
...
- After Export, every entry with `_selected[key] == True` either lands
  on disk at the expected path OR appears in the per-entry results
  list with a non-None error message.
...
- End-to-end: selected entries land at
  `<destination>/<source>/<TICKER>_<INTERVAL>.csv`; unselected entries
  do NOT land.
