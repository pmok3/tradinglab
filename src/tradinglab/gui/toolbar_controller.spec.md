...
- **Compare on/off is a fixed-width toggle button, not a checkbox** (audit `compare-toggle-button`). `compare_check` is a `ttk.Button` (`width=12`, themed `TButton` — `Toolbutton` is intentionally avoided because it isn't in `build_ttk_style_spec` and would render unthemed in dark mode). Its `command` (`_on_compare_button`) flips the `compare` BooleanVar (still the source of truth read by `_render` / `_on_compare_toggle` / topology key everywhere) and then calls `on_compare_toggle` — mirroring a checkbox's flip-then-command order. A trace (`_bind_compare_button`) keeps the label `Compare: On` / `Compare: Off` in sync whether toggled by the button or set programmatically (failed-compare revert, sandbox replay). The attribute name `compare_check` is retained for back-compat even though the widget is now a button.
