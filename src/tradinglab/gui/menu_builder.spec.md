...
- Move the large menubar-construction block out of `app.py` while preserving TradingLab's existing menubar surface across these top-level cascades, **in display order**:
  ...
  6. **View** — Heikin-Ashi (cascade: Show Heikin-Ashi Candles + Highlight Flat Bars), Highlight Key Bars, Volume time-of-day shading, Ratio charts (A/B), ChartStack (cascade: Show ChartStack + Settings…), Live Market Heatmap, Heatmap (Finviz).
