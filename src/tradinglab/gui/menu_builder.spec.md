...
- **Ratio charts (A/B) submenu** (audit `ratio-render-modes`). `View → Ratio charts (A/B)` is a cascade (`_ratio_menu`, registered in `_submenus` for dark-theme + cleanup) with two checkbuttons — *Show as candlesticks (+ volume)* (`_ratio_candles_var` → `_on_menu_toggle_ratio_candles`) and *Rebase to 100* (`_ratio_rebase_var` → `_on_menu_toggle_ratio_rebase`). Both only affect ratio symbols (AMD/NVDA, ...); see `app.spec.md` → "Ratio render modes". Persisted + restored like the other View toggles.
