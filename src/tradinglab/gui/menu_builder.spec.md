...
- **Quant, Live Market Heatmap and Heatmap (Finviz) are one market-context group.** All three answer "what is the market doing", as opposed to every View entry above them, which changes how the **active chart** is drawn — hence the separator after the ChartStack cascade. `Quant` is a `add_checkbutton` (bound to `_quant_visible_var` / `_on_view_toggle_quant`) rather than a command, because the Quant side tab is a panel the user pulls up and puts away, not a one-shot action. It takes no ellipsis: per `tests/unit/gui/test_ellipsis_semantics.py` that is reserved for dialogs, and this reveals a tab. The callback lives on [`QuantAppMixin`](quant_app.spec.md) and is declared on the `MenuBuilderCallbacks` protocol next to `_on_view_toggle_chartstack`; the var is not persisted (see [`app_state.spec.md`](app_state.spec.md)). The toggle previously sat under Tools and moved to View so it groups with the other market-context surfaces.
- **The View menu has two heatmap entries, and they say which is which.** Audit `view-heatmap-launcher` (2026). `View → Heatmap (Finviz)` hands off to `webbrowser.open("https://finviz.com/map.ashx?t=sec", new=2, autoraise=True)` — the Finviz S&P 500 sector treemap (1D performance). `View → Live Market Heatmap` opens the in-app window ([`gui/sandbox_heatmap.open_live_heatmap`](sandbox_heatmap.spec.md)) on the current tape. The `(Finviz)` qualifier was added when the in-app entry landed: two items both reading "Heatmap" would be indistinguishable. Neither takes an ellipsis — per `tests/unit/gui/test_ellipsis_semantics.py` that is reserved for dialogs, and these open a browser and a window respectively. The Finviz fallback when the OS browser hand-off fails is a `messagebox.showinfo` containing the URL so the user can copy-paste it manually. Both callbacks live on [`ViewMenuMixin`](view_menu.spec.md) (extracted from `app.py` under §7.24) and are declared on the `MenuBuilderCallbacks` protocol next to `_on_view_toggle_chartstack`.
