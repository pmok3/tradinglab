...
- `class _SettingsDialog(BaseModalDialog)`:
  - Shows a worker-thread spinbox, a **Display & Appearance** section (Dark mode, Logarithmic price axis, Invert scroll-zoom direction, Volume time-of-day shading, UI scale, and Use color-blind-safe palette), a **Drawings** section (Snap horizontal lines to nearest OHLC), a **Sandbox** section (Reference symbol and Skip detailed journal), a **Startup parameters** `LabelFrame`, a **Display timezone** `LabelFrame`, a **Theme customization** hint frame whose body is a short label + an `Open Theme Editor…` button, and a **Watchlist** section (Pinned sub-tab cap). The Startup section also contains `Show splash screen on startup` and `Check for updates on startup`. The per-slot color grid (light/dark swatches + presets) moved to the dedicated `ThemeEditorDialog` (`gui/theme_editor.py`, big-bet item #7); the button here is a one-click fallback for users who reach Settings out of habit. `_swatch_buttons` is kept as an empty per-mode dict for back-compat with legacy tests that introspect it.
  - **Startup parameters** has one row per entry in `constants.STARTUP_DEFAULT_KEYS`. `interval` / `source` / `theme` use readonly `ttk.Combobox` (values pulled from `INTERVAL_PERIODS.keys()`, `data.user_visible_sources()`, and `("light","dark")` respectively — note: `user_visible_sources()` not `DATA_SOURCES.keys()`, so synthetic / synthetic-stream are filtered out per `data/base.spec.md`; the source combobox shows raw keys in registration order, e.g. `yfinance`, `Auto`, then credential-gated `alpaca` / `yfinance+alpaca`); `ticker` / `compare` use `ttk.Entry`. Two helper buttons sit below the rows: **"Use current chart"** (`_on_capture_current_as_default` — copies the live `ticker_var` / `compare_ticker_var` / `interval_var` / `source_var` / `dark_var` values into the dialog's StringVars) and **"Reset to builtins"** (`_on_reset_startup_defaults` — sets every dialog StringVar back to `BUILTIN_STARTUP_DEFAULTS`, whose source default is `Auto`). A muted hint label clarifies "Applied next launch — does not change the current session." The same section includes the splash checkbox and the update-check-on-startup checkbox. (The update endpoint URL is intentionally not user-facing — it resolves from the `UPDATE_CHECK_URL` env var / built-in GitHub Releases default via `updates._resolve_url`; the `update_check_url` setting key still exists for power users editing `settings.json`/env directly.) Persistence happens at "Save and Close" (`_commit_startup_defaults` walks `STARTUP_DEFAULT_KEYS` and calls `parent.set_startup_default(key, value)` for each; `_on_ok` writes the splash/update tunables).
  ...
  - Dark-mode, log-price, scroll-zoom invert, snap-to-OHLC, UI-scale, color-blind palette, and volume time-of-day toggles **preview live** (via their `_on_*` handlers). Startup-parameter edits, sandbox reference/skip-journal edits, splash/update-check edits, watchlist-cap edits, and timezone selection are **not** live in the dialog — they only land at "Save and Close". (Timezone is then applied immediately by `set_display_tz`'s render+refill.) **Cancel reverts** live-preview state to the initial state (stored in `_dark_initial` / `_log_initial` / `_scroll_invert_initial` / `_snap_ohlc_initial` / `_ui_scale_initial` / `_colorblind_initial` / `_vol_tod_initial` / `_overrides_initial` / `_startup_initial` / `_tz_initial`, the override and startup snapshots being a `deepcopy` of the parent dicts captured at dialog open). Cancel is wired to both the button and `WM_DELETE_WINDOW`. The primary footer button is labelled **"Save and Close"** (renamed from "OK" in the dialog-paradigm sweep — audit `dialog-button-paradigms`) and commits the worker count, the startup-defaults dict, sandbox/defaults tunables, splash/update-check tunables, watchlist cap, UI scale, and (if changed) the display timezone; theme override state was already applied live.
...
- `class _WatchlistDialog(BaseModalDialog)`:
  ...
  - Buttons: `New`, `Rename`, `Delete` (for names); `Pin`, `Unpin` (toggle pin state of the selected list); `Add`, `Remove` (for tickers); `Import…`, `Export…` (JSON import via `watchlists.import_from_file`, mode `"merge"`; export via the manager's `save_to_file`).
...
- Live preview for theme, log-price, scroll-zoom invert, snap-to-OHLC,
  UI-scale, color-blind palette, and volume time-of-day settings
  (instant feedback). Cancel reverts by re-applying the initial snapshot;
  OK persists the live-previewed values where needed. Worker count is
  committed on OK only.
...
- `_SettingsDialog._on_cancel` fully reverts dark mode, log-price preview,
  ...
  `_overrides_initial` / `_startup_initial` snapshots — `deepcopy` of
  parent dicts).
- `_SettingsDialog._on_ok` writes the worker count exactly once and commits
  ...
  It also persists the scroll-zoom invert, snap-to-OHLC, UI-scale,
  sandbox reference/skip-journal, splash, update-check, watchlist-cap,
  and display-timezone settings. Theme overrides were persisted live;
  OK is a no-op for them.
