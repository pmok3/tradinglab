...
- `_apply_menubar_theme` delegates to `gui.menu_theme.apply_menu_theme`, which sets every classic `tk.Menu` colour option plus `borderwidth=0` / `relief="flat"` on the menubar and recursively on cascade submenus. This keeps Windows Tk on the themed rendering path so submenu arrow indicators inherit `theme["text"]` in dark mode. `disabledforeground` also uses `theme["text"]` because Windows Tk can draw resting cascade arrows through the disabled-foreground path; `activeforeground` covers only hover/highlight. Audits `menu-disabled-fg`, `menu-cascade-arrow-dark`.
...
- `_apply_treeview_row_tags` covers the watchlist trees, primary/compare OHLC tables, and (forward-looking, via `getattr(tab, "_tree", None)`) the Entries and Exits strategy trees. Today the latter pair don't `tag_configure("bull"/"bear")` their rows, but registering the palette now keeps every Treeview the controller owns reachable in one pass so a future bull/bear tinting feature lands without a controller change.
