...
The six scalar comparisons (`_eval_gt`, `_eval_lt`, `_eval_ge`, `_eval_le`,
`_eval_eq`, `_eval_ne`) are generated by a single `_binary_cmp(compare)`
factory driven by the stdlib `operator` module (`operator.gt`,
`operator.lt`, …) — they were six byte-identical bodies differing only in
the final compare. Behaviour (including the Kleene rule: either side
resolving to `None` for a missing bar / unhydrated indicator /
unresolvable cross-symbol ref makes the whole comparison `None`, not
`False`) is identical, and the `OPERATOR_EVALUATORS` keys/contents are
unchanged. The windowed / stateful operators (`between`, `crosses_above`,
`crosses_below`, `is_rising`, `is_falling`, `within_pct`, `inside_bar`,
`nr7`, …) are hand-written and untouched. New stdlib dependency:
`operator`.
