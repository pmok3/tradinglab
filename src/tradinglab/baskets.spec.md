...
- `quant_symbols() -> List[str]` — returns the fetchable legs behind the Quant tab catalog via `quant.catalog.quant_leg_symbols()`. It returns legs, not ratio rows, because ratios are derived and never persisted.
...
- `BUILTIN_BASKETS: Dict[str, Callable[[], List[str]]]` — keyed registry: `{"sp500", "qqq", "nyse", "nasdaq", "quant"}`. The keys are stable IDs used in the manifest sidecar; renaming a label must not break old manifests.
- `BUILTIN_BASKET_LABELS: Dict[str, str]` — display labels for the GUI. Decoupled from `BUILTIN_BASKETS` keys deliberately.
- `BUILTIN_BASKET_REFRESHED_DATES: Dict[str, str]` — per-basket snapshot dates so dialogs can render a "refreshed YYYY-MM-DD" suffix without reaching for module-level constants. SP500 is intentionally absent (Wikipedia-derived CSV has no baked-in date); `quant` is absent because it is generated from code, not a snapshot.
...
- `NON_EQUITY_BASKETS: frozenset` — `{"quant"}`. Used by the prepare-universe dialog to disable the fundamental filter for non-company gauges.

## Dependencies
- Internal: `_resources.resource_path` (frozen-mode-aware `tools/*.csv` path
  resolution), `quant.catalog.quant_leg_symbols` (lazy import inside
  `quant_symbols()`).
...
- **Quant is generated, not refreshed.** `quant_symbols()` is derived from
  `quant/catalog.py`, which is already the source of truth for market
  internals. It is deliberately absent from `BUILTIN_BASKET_REFRESHED_DATES`
  because there is no CSV snapshot date to go stale.
- **Quant has no survivorship caveat.** `FULL_EXCHANGE_BASKETS` omits
  `quant` because it is a curated gauge list, not point-in-time index
  membership. `NON_EQUITY_BASKETS` marks it separately so the prepare dialog
  can disable fundamental filters that only make sense for companies.
- **Quant uses legs, not rows.** Ratio rows cannot be persisted by
  `disk_cache`, so a preload basket must contain the vendor-fetchable legs
  returned by `quant_leg_symbols()`.
...
- `NON_EQUITY_BASKETS` is a subset of `BUILTIN_BASKETS` keys.

## Testing
- `tests/unit/test_quant_universe.py` pins the Quant basket registry, label,
  non-equity classification, lack of refreshed date, and lack of
  full-exchange classification.
