...
- `remap_window_by_time(prev_dates, prev_xlim, new_dates) -> Optional[(lo, hi)]` — remap an index-space xlim from a previous series onto a new series by timestamp coverage. Returns a half-open slice or `None` for invalid / degenerate remaps. The source-window bar indices are taken as the FULLY-visible bars — `lo_i = ceil(lo_f)`, `hi_i = floor(hi_f)` — NOT `round()`. `round` is banker's-rounding and was WRONG for the common HALF-INTEGER xlim: a drilled day's xlim is `(lo-0.5, hi+0.5)`, and `round(hi+0.5)` yields `hi+1` for ODD `hi`, pulling the NEXT day's first bar into the remapped window (the "extra bar on the right after a compare toggle on a drilled day" bug, audit `remap-window-halfbar-round`). `ceil(lo-0.5)=lo` and `floor(hi+0.5)=hi` land on exactly `[lo, hi]` regardless of parity. Returns `None` when the source window spans the **entire** source series (`lo_i == 0` AND `hi_i == n_prev-1`) — viewing all of a symbol (e.g. a 2-bar IPO showing its full history) is not a deliberate zoom, so there is no calendar selection to preserve; the caller falls back to its default right-edge window. A proper sub-window (touching at most one edge), however narrow, is still remapped. **Tz-robust**: both sides are normalized via `_tz_naive_key` (strip tzinfo → ET wall-clock) before comparison, and the comparison loop is wrapped to return `None` on any residual ordering error — a tz-mixed (aware vs naive) input must never raise.
...
- **Tz-robust comparison** (`remap_window_by_time`): `prev_dates` and `new_dates` can arrive with different tz-awareness — a freshly-fetched tz-aware **ET** series (yfinance, and — since the Alpaca/Schwab/Polygon ET-localization fix — every live source) vs. a previously-rendered series that may be tz-naive or a legacy tz-aware cache. A bare `d <= t_lo` on a tz-aware/tz-naive pair raises `TypeError: can't compare offset-naive and offset-aware datetimes`, which previously propagated out of `ChartApp._render` and **silently dropped the compare panel** on a ticker switch. Fixed by `_tz_naive_key` (strip tzinfo → ET wall-clock, matching `core.pairing._normalize_pairing_key`; the codebase treats a naive intraday timestamp as ET) applied to both sides, plus a `try/except TypeError → None` around the loop. `ChartApp._compute_slot_window` also wraps the call defensively so window-remap math can never tear down the whole render. Pinned by the tz-mixed cases in `tests/unit/test_viewport_remap.py`.
...
- `remap_window_by_time` never raises on tz-mixed (offset-aware vs offset-naive) inputs — keys are tz-normalized before comparison; any residual ordering error degrades to `None`.
