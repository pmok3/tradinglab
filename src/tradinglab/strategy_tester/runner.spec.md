...
- **RTH-only filter (default):** the `_worker` slices candles to the configured date range, then — when `cfg.include_extended_hours is False` (the default) — drops every bar outside US-equity Regular Trading Hours via `_filter_rth_only`. RTH = Mon-Fri AND 09:30 ≤ ET time ≤ 16:00. **Vectorized:** `_filter_rth_only` builds the timestamp array once and delegates the RTH-membership decision to `evaluator._compute_et_arrays` (one numpy pass + ~250-500 zoneinfo probes per UTC-day-range) instead of calling `_is_regular_session(_bar_ts_to_et(ts))` per candle. For a year of 5m bars (~25k candles per symbol) this trims seconds of per-symbol setup time. Output identical to the prior per-candle implementation including DST transition days. Indicators (EMA, SMA, RSI, VWAP, ...) computed inside the evaluator therefore see only RTH bars by default and aren't skewed by premarket / postmarket prints. Opt in via `TestConfig.include_extended_hours=True` (GUI checkbox in `strategy_tab.py`). Saturday-only inputs collapse to an empty candle list — the evaluator handles this without crashing.
- **Indicator warmup pre-load:** before fan-out, `run()` calls `warmup.required_warmup_bars(entry, exit)` to compute how many bars every referenced indicator needs to be fully hydrated, then `warmup.bars_to_calendar_days(bars, interval)` to convert that to a calendar-day window. The fetcher is invoked with `fetch_start_date = start_date − warmup_calendar_days` (so the candle stream actually contains the warmup bars), and the `_worker` passes `warmup_until_ts = midnight-UTC(start_date)` into `evaluate_symbol`. The evaluator ticks the engine through the warmup window (indicators hydrate, position state stays consistent) but blocks all entry/exit trigger firings and EOD-kill synthetic fills, then trims `SessionResult.equity_curve` to the active period before return. `TestConfig.warmup_override_days` (positive int) skips the auto-compute and uses an explicit calendar-day count instead; `None` (default) means auto. **Smoke-stub safety:** if the fetcher returns no extra warmup bars (its data starts at-or-after `start_date`), the worker degrades to `warmup_until_ts=None` so legacy fixtures with deliberately-narrow stub data keep working without modification.
- **disk-cache for fetches:** `fetch_candles_for_symbol` routes through `tradinglab.disk_cache` (the same JSONL-on-disk cache the live chart loader uses) keyed by `(source="yfinance", ticker, interval)`. On a hit the fetcher is never invoked; on a miss the fetcher's output is `merge_candles`d onto whatever is already on disk and persisted atomically. A per-`(source, ticker, interval)` `threading.Lock` (module-level dict in `runner._fetch_locks`) coordinates concurrent workers so two threads requesting the same symbol cannot double-fetch or torn-write the same JSONL file. Sealed OHLCV bars are immutable, so the cache has no TTL — staleness is handled at the live-chart layer by `ChartApp._cache_is_stale`, but for batch strategy_tester Runs the user explicitly chose a historical date range and the cached bars are exactly correct for it. This eliminates the previous "every Run re-downloads 12 symbols × 1-3s = 12-36s of yfinance latency" worst case; repeat Runs on the same universe are network-free.
- **per-symbol parallel screenshot pool:** `_render_screenshots_for_symbol` now fans each closed trade out to a small `ThreadPoolExecutor(max_workers=min(4, n_trades), thread_name_prefix="shots-<SYM>")` instead of looping sequentially. Safe because `render_trade_screenshot` builds a fresh `Figure()` + `FigureCanvasAgg` per call (no global `pyplot`). The 4-worker cap stops us from oversubscribing the CPU when the outer `_default_max_workers` pool is already running 8-12 symbol workers in parallel. Cancellation: `_render_one` short-circuits on `cancel_token.is_cancelled()` so queued tasks for a cancelled Run don't keep writing PNGs. Output filenames and screenshot content are unchanged; only the order they hit disk may interleave across trades.
- **cancel-token threading:** `_worker` passes its `cancel_token` through to both `evaluate_symbol(..., cancel_token=...)` and `_render_screenshots_for_symbol(..., cancel_token=...)`. After evaluation, if the token is tripped, the outcome is recorded with `ok=False` and `error="cancelled mid-evaluation"`. The orchestrator's `cancelled_mid_run` short-circuit still takes precedence in the final-status decision, so a CANCELLED Run remains CANCELLED (never re-promoted to FAILED) regardless of per-outcome `ok` flags.
...
- `tests/unit/strategy_tester/test_fetch_caching.py` — disk-cache miss-then-hit invariant, distinct-key isolation, byte-identical round-trip, empty-result no-poison.
- `tests/unit/strategy_tester/test_parallel_screenshots.py` — every trade gets a PNG, cancel-token short-circuits, no-spec / no-trades no-ops, optional perf smoke (skipped under `CI`).
