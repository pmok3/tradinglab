...
- `collect_interval_overrides(entry_strategy, exit_strategy, test_interval) -> list[str]` — returns one warning string per authored trigger / condition / field interval that differs from the Strategy Tester run interval and will be rewritten by the single-interval normalizer. The runner forwards this list into the report aggregate so the HTML/PDF/report UI can explain the rewrite.
...
2. **ET conversion**: `_compute_et_arrays(bars.ts)` runs once per symbol,
   returning `(et_date_ints, rth_mask, et_offsets_sec)` for UTC epoch-second
   timestamps. Dates are days-since-epoch in ET; RTH is Mon-Fri 09:30-16:00.
   Each UTC day is probed at both ends; unchanged offsets broadcast to its
   bars, while DST-transition days resolve offsets per bar. The vector
   arm-window gate uses `(bars.ts + et_offsets_sec) % 86400` with inclusive
   boundaries, avoiding its per-bar datetime construction and parsing.
   Scalar arm-window evaluation still uses `_bar_ts_to_et(ts)`. Scalar
   exit handlers retain dated Bar adapters in both modes; TIME_OF_DAY
   cutoffs explicitly compare ET. `require_market_open` uses `rth_mask`
   in both modes. See `test_vectorized_et_arrays.py` for DST/boundary parity.
...
There is one bar loop and one copy of `_check_entry`, `_check_exits` and
activation logic. Engine ticks, fills, EOD handling, caps, cooldown, warmup,
cancellation, STACK and partial exits do not branch into separate loops.
Only optional decision kernels and time facts differ between modes.

- Entry MARKET and supported INDICATOR/SCANNER_ALERT predicates use
  dispatch-owned masks. Entry price triggers stay scalar.
...
- Handler identity is checked at both kernel creation and consumption.
  ...
  Scanner-alert edge updates share the scalar dispatch helper and occur
  only after entry gates pass.

Dtypes: bar timestamps `int64`, OHLC `float64`, ET offsets `int64`,
seconds-since-ET-midnight `int64`, all decision masks `bool`.

Complete `SessionResult` equality (including fills, equity, cash and
dtypes) is pinned against scalar mode by `test_vectorized_agreement.py`.
The performance contract is `tests/perf/test_strategy_eval_perf.py`:
min-of-7 interleaved wall-clock on 4,056 bars must clear the unchanged
1.5x speedup gate. Timings are machine-dependent, not a universal 2.9x claim.
...
## Dependencies
- `.vector_eval` — optional dispatch kernels and entry time facts, consumed by the single orchestration loop (see `vector_eval.spec.md`).
...
- `tests/perf/test_strategy_eval_perf.py` (`@pytest.mark.perf`) —
  ...
  (machine-specific timings are printed by the test).
