...
- `@dataclass(frozen=True) class TestConfig` — full envelope: entry_strategy_id, exit_strategy_id, universe, start/end_date, interval, starting_cash, cost_model, date_preset, rng_seed, schema_version, user_label, include_extended_hours, warmup_override_days.
...
- **`warmup_override_days` defaults to `None` (auto-compute)** — when `None`, the runner walks the strategies' indicator references via `strategy_tester.warmup.required_warmup_bars` and pre-loads enough history before `start_date` for every indicator to be fully hydrated by Day 1 of the active period. When a positive int, the runner uses that exact calendar-day count and skips the auto-compute. Missing key in old JSON manifests deserialises to `None` for back-compat (= auto-compute path).
