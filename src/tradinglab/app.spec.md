...
- `gui/{drilldown,interaction,workers,watchlist_tab,indicator_menu,sandbox_menu,entries_app,exits_app,help_menu,banner,config_menu,anchor_pick_app,drawings_app,live_price_overlay_app,recent_menus,scanner_app,snapshot,update_check}.py` and `backtest/{sandbox_app_aliases,sandbox_app_methods}.py` — other mixins (`AnchorPickAppMixin` in `gui/anchor_pick_app.py` — the AVWAP "Pick Anchor…" click flow; `ScannerAppMixin` in `gui/scanner_app.py`; `SandboxAppMixin` in `backtest/sandbox_app_methods.py`).
...
**Gates, scoped narrowly to avoid regressing manual interior zoom:** `use_time_preserve` = `in_drilldown` (`_drilldown_day` set AND intraday) AND `compare_on` AND `is_historical` (visible right edge sits before the primary's last real bar). This is the OUTER gate — it engages the TIME-preserve for both failure modes. The INNER gate `compare_lacks` = `use_time_preserve` AND `_compare_cache_earliest_gt(lo_ts)` (compare HAS cached data starting AFTER the window — an EMPTY cache returns False, so a fresh compare uses the ordinary cache-miss load path) additionally engages `keep_window` + background-fill. The drilldown scope is deliberate: a normal manual rubber-band zoom keeps the existing index-preserve (`check_d52` clears its inherited `_drilldown_day` to stay on that ordinary path). The manual-pan-into-history case is a deferred follow-up. The toggle body is extracted into `_apply_compare_toggle(compare_on, *, keep_window=None)` (behaviour-identical) so both the gated path and `_on_compare_fill_done` can invoke it. Instance state: `_compare_fill_inflight: set`, `_compare_fetch_token: int` (init in `__init__`). Pinned by `check_d86` (deep-history no-jump + fill-on-completion) and `check_d87` (in-coverage grid-mismatch: sparse primary + dense compare inserts gaps → time-preserve holds the drilled day as the left edge; fails without the broadened `use_time_preserve` gate).
