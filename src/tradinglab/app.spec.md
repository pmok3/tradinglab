...
6. `_prefetched_raw` ingests executor-fetched bars from `_load_data_async` or the poll tick without a second
   provider call. Its worker payload may include disk-preloaded and pre-merged/pre-saved primary/compare lists so `_load_data` can skip Tk-thread JSON parsing, `merge_candles`, and `disk_cache.save`. A worker-side save failure (detected via `save`'s `False` return, logged as a warning) drops the pre-merged list so `_load_data` re-merges instead of assuming the write landed — preserving the "pre-merged set ⟹ on disk" invariant the Tk phase relies on to skip its own save. The worker's per-side merge+save is itself guarded by `disk_cache.merge_adds_nothing(disk, merged)`: when the trailing fetch adds nothing new (fully pre-downloaded / sealed universe — the common case when browsing the cached S&P/Nasdaq universe), the ~450 ms rewrite of the multi-MB 5m JSONL is skipped (the in-memory merged list is still returned for render). When it supplies fresh primary/compare data,
...
- **Shared per-slot helpers (Stages 3–5)** — both paths call ONE implementation each: `_draw_slice` (detach + rebuild candle/volume/shading/indicator/event/vol-ToD), `_compute_slot_window(slot, ax_p, candles, *, preserve, preserved_xlim, slide_to_right, prev_primary_dates, prev_primary_xlim, primary_window=None, primary_applied_xlim=None) -> (lo, hi, xlim_set)` (ticker-switch time-remap / drilldown preserve+slide / right-edge default — returns whether it already applied the xlim), and `_paint_slot_watermark(slot, ax_p)` (centered ticker watermark). The fast path passes `preserve=False`/`preserved_xlim=None`/`slide_to_right=False` (those cases are dispatch-excluded); time-remap rides on `prev_primary_dates`/`xlim` (None ⇒ skipped).
- **Compare slot mirrors the primary's EXACT applied xlim (shared x-axis).** Every slot's price/volume/indicator axes are created with `sharex=ax_p1`, so a `set_xlim` on ANY of them moves ALL. The ticker-switch time-remap is **primary-only** (it maps the *previous primary's* visible calendar window onto the new symbol). Both render loops therefore capture the primary slot's resolved `(lo, hi)` (as `primary_window`) AND — when the helper applied the xlim itself (`xlim_set` True) — the primary axis's actual float xlim (`primary_applied_xlim = ax_p.get_xlim()`), passing both to the **compare** slot; `_compute_slot_window` re-applies `primary_applied_xlim` **verbatim** for any non-primary slot (falling back to the integer window `(p_lo-0.5, p_hi-0.5)` only when it's None — the default/right-edge path, where the primary applies its own xlim AFTER the draw and the integer reconstruction already agrees). Without the exact-mirror, the compare slot reconstructed the shared xlim from the floored/ceiled integer window: under an **index-preserve** render the primary applies a fractional `(k+0.5, m+0.5)` but `primary_window` is `(floor, ceil) = (k, m+1)`, so the compare's `(k-0.5, m+0.5)` shifted the shared LEFT edge left by 1.0 EVERY render. Single renders looked fine, but the market-hours next-bar **poll tick force-arms `_preserve_xlim_on_render` and re-renders every few seconds**, so the window marched one bar left per poll — the "candles incrementally creep in from the left" bug, provider-agnostic (nothing yfinance/Alpaca-specific) and invisible to non-polling repros. Also fixes the earlier compare-mode ticker-switch clobber (compare fell to the right-edge default and reset the shared axis to the prior session). No-op for single-chart mode. Audit `compare-slot-xlim-mirror`; pinned by `check_d90` (repeated index-preserve renders with compare ON keep the left edge + width pinned).
...
- `_preserve_xlim_on_render` — capture xlim before clear, restore after. Never auto-cleared at end of `_render`; reset only by explicit user intent: `_reset_view`, `_do_scheduled_reload`, and `_on_explicit_axis_change`. On an **interval** change the previous interval's bar-index xlim is meaningless, so both preserve flags drop and the new series snaps to the right-edge default. On a **source-only** change (same ticker + interval) `_on_explicit_axis_change` instead sets `_preserve_xlim_by_time_on_render=True` (index-preserve OFF) so the visible DATE window is remapped onto the new provider's series — two providers can return different-length series (yfinance 60d-5m vs Alpaca 120d-5m), so reusing the bar-INDEX window would jump the view to a different calendar day (audit `source-switch-view-preserve`).
...
policy, and watchlist OHLC warming. `_load_data` keeps the foreground sync
render path and non-OHLC watchlist refreshes (events/signals) but does not
submit reactive OHLC prefetch jobs itself. Legacy `_ensure_prefetched` remains
for live polling and on-demand overlays that are explicitly outside the
scheduler cut-over.

### Today's-bar upsampling on the daily chart
Most data providers lag today's daily bar until after the close, so a mid-session user on a 1d chart sees "everything up to yesterday" while the 5m chart shows the live forming bar. `_maybe_upsample_today_daily(candles, source, symbol, interval)` layers a synthetic today-bar onto a daily series by aggregating whatever intraday data is already cached (finest interval wins — see `data/today_upsample.find_best_intraday_source`). Called from `_load_data` AFTER the truthful cache store (so `_full_cache` keeps the provider's raw lagged data, ready to overwrite the synth bar on the next render boundary) and from the compare-on cache-hit branch of `_on_compare_toggle`. When an intraday scheduler prefetch lands, `_refresh_daily_synth_for_active_view(prefetched_symbol=...)` re-runs the upsample + pair-filter + render path (no network, no indicator-cache clear — forming-bar invalidation via `_invalidate_focused_panels` covers the right edge). **Compare-panel preservation (`daily-synth-compare-drop`):** this refresh reads the compare's daily bars from the bounded in-memory `_full_cache` (`full_cache_size`, default 16); when the compare was LRU-evicted the lookup misses. It MUST NOT blank an on-screen compare — the else-branch `compare=[]` would drop the whole compare panel (`self._compare` empty → `_render` omits the slot). So on a memory miss it falls back to `disk_cache.load` (the truthful persistent store) and re-warms `_full_cache`; if disk is ALSO empty it preserves the currently-rendered `_compare_raw`; only a genuinely-empty compare (memory+disk+on-screen all empty) blanks. The primary side already guarded this via its `if not primary_clean: return` early-out. The polling tick on 1d redirects to a 5m prefetch (see `gui/polling.spec.md`). Missing intraday data is now warmed by the scheduler dual-interval policy; `_maybe_upsample_today_daily` no longer submits a self-heal companion prefetch itself, which removes the historical prefetch→refresh→prefetch livelock hazard. Scope: 1d only; 1wk/1mo deferred (see `data/today_upsample.spec.md`). Audit `daily-today-upsample`.

### Bad-ticker handling
Revert StringVar to `_confirmed_*_ticker`. Status copy is composed by `_ratio_failure_message(raw)`, which distinguishes four shapes (vendor name omitted intentionally throughout):
- **Plain ticker** — `Ticker '{raw}' not found. Check the spelling or try a different data source.` (the §12 smoke check matches on `not found`).
- **Quotient ratio** (`AMD/NVDA`) — `Ratio '{AMD / NVDA}' could not be loaded. Check that both legs are valid tickers`.
- **Scaled symbol** (`^VIX/15.87`) — `Scaled symbol '{^VIX / 15.87}' could not be loaded. Check that '^VIX' is a valid ticker`. Only the base can be wrong, so pointing at "both legs" would misdirect.
- **Unsupported numeric shape** (`^VIX/0`, `100/VIX`, `16/4`) — `'{…}' is not a supported form. Divide a symbol by a positive number, e.g. ^VIX/15.87`. Telling someone who typed `^VIX/0` to check that both legs are valid tickers sends them hunting for a ticker named `0`.

The centered chart **watermark** (`_paint_slot_watermark`) and the **window title** render ratios via `ratio_display_label` (`AMD / NVDA`, `^VIX / 15.87` — the divisor is always visible so a scaled chart is never misread as raw).

### Index-symbol re-resolution (`_reresolve_symbols_for_source`)
Index symbols are spelled differently by every vendor (`^VIX` yfinance / `$VIX` Schwab / `I:VIX` Polygon — see `data/index_aliases.spec.md`). `_reresolve_symbols_for_source` rewrites `ticker_var` and `compare_ticker_var` into the ACTIVE source's vocabulary and is called at the top of both `_load_data` and `_load_data_async`, including inside their sandbox early-return branches before either branch reads the ticker box. Without the sandbox call, a Quant row registered literal `VIX` while the prepared universe and disk cache held `^VIX`. Because `index_aliases.resolve_symbol` canonicalises its input first and is idempotent, that one seam serves both entry points: typing `VIX` rewrites the box to `^VIX`, switching source re-resolves `^VIX` → `$VIX`, and plain tickers are no-ops. No separate source-change hook exists, so there is no second copy of the rule to drift. Ratio legs resolve independently and a scale constant is preserved (`VIX/15.87` → `^VIX/15.87`). Best-effort: never raises into a load path. Unit-tested in `tests/unit/gui/test_source_change_reresolve.py` and `tests/unit/test_quant_universe.py`.
...
- **In-readout overlay legend** (replaces the retired Tk overlay pill): `ChartApp.__init__` sets `self._overlay_legend = None` and `self._overlay_legends = {}`; no Tk overlay widget is constructed. The overlay legend is now rendered as transparent matplotlib `TextArea` rows inside the top-left readout offsetbox by `InteractionMixin` (`_build_readout_indicator_rows` / `_update_readout` / `_maybe_handle_readout_legend_click`). B1 on a legend row routes to `_open_per_indicator_dialog(config_id, slot)` and B3 to `_show_legend_context_menu(...)`. `_refresh_overlay_legend` / `_reposition_overlay_legends` / `_on_theme_changed` are compatibility no-ops.
...
Settings dialog → "Startup parameters" sub-frame. Builtins: `constants.BUILTIN_STARTUP_DEFAULTS` (AMD / SPY / 1d / Auto / light). Stored sparsely under `settings.json["startup_defaults"]`. `constants.resolve_startup_defaults(...)` validates per-key (interval / source allow-lists, theme ∈ {light, dark}, ticker upper-strip). The active startup source can additionally be forced by the `TRADINGLAB_STARTUP_SOURCE` env var (honoured in `AppState._resolve_source`; power-user knob + test seam). Public: `set_startup_default` / `clear_startup_defaults` / `replace_startup_defaults`. Changes apply on next launch — **except `theme`, which additionally applies live when a config is loaded** (audit `config-theme-roundtrip`; see "Light/dark theme round-trip" below).

### Display timezone
Settings dialog → "Display timezone" combobox. Stored under `settings.json["display_tz"]`, read into `self._display_tz`. Used by `formatting.format_dt(...)` at two intraday display sites: x-axis `%H:%M` ticks and `_format_candle_date`'s intraday branch (which drives the hover tooltip). Daily+ never converts (a daily bar is a date label, not an instant). `set_display_tz(tz_name)` persists and clears `_SeriesArrays._tooltip_cache`. Bad IANA names silently fall through to raw `strftime` via `format_dt`'s try/except.
...
**Ticker switch preserves the visible window ONLY when the view is historical (`ticker-switch-default-view-align`).** The non-drilldown ticker-switch paths — `_do_scheduled_reload` (typing / click-to-type), `_cycle_watchlist_ticker` (Space), `_on_watchlist_double`, `_on_chartstack_promote` — set `_preserve_xlim_by_time_on_render = self._ticker_change_should_time_preserve()` instead of an unconditional `True`. The helper returns `_current_visible_window_ts()[2]` (`is_historical`): **True** only when the user has panned/zoomed back into history (carry that calendar window onto the new symbol), **False** at the default right-edge view (e.g. right after **Reset View**, or just flipping a watchlist) so the new ticker snaps to ITS OWN default window. Without the gate, time-remapping the previous ticker's window onto the new one misaligns the new ticker's left edge when the two have different-length / different-density histories — a sparse small-cap's wider reset window (last-200 of a gappy series) shoved a dense large-cap's leftmost bar back several months (the "AREC → Space → AMD is not aligned, leftmost jumped from 2025-09 to 2025-08" report). Returns False on any error / not-ready panel (safe default = show the new ticker's own window). Pinned by `check_d91` (default-view switch shows the new ticker's default; panned-view switch preserves the calendar window).

**`_on_compare_toggle` is view-safe — the primary NEVER jumps or shifts (`compare-toggle-drilldown-preserve`).** Two distinct failure modes were folded into one fix:

* **Jump (deep history):** enabling compare while drilled into a day the compare's cached 5m history doesn't reach (deep-history providers cap intraday depth — Alpaca 5m ≈ `_DEEP_HISTORY_INTRADAY_DAYS["5m"]` = 120 days) snapped the chart to the recent right edge, because `core.pairing.align_pair`'s low-end intersection (`lo_day = max(p0, c0)`) DROPPED the drilled day from BOTH series → "jumped to June 5th".
* **Leftward shift (in-coverage day):** enabling compare while drilled into a day the compare DOES cover still dragged the view ~1 day to the LEFT — the drilled day's open ended up mid-screen and the PRIOR day became the left edge. Root cause: `align_pair` inserts `Candle.gap` slots wherever the primary and compare 5m grids differ (IEX serves sparse bars — SPY has timestamps AMD lacks and vice-versa), SHIFTING the drilled day's absolute index in the aligned primary (e.g. +59 for AMD/SPY on 2026-05-08). Naive index-preserve then reuses the stale index xlim.

The prior fix (disk-preload the compare's full history) could not solve the jump when disk itself lacks the old compare data, and did nothing for the shift. Replaced by a design with THREE layers, engaged by two nested gates:

1. **Preserve the view by TIME, not index — engaged for EVERY formal intraday drilldown (including the newest/right-edge session) and every covered historical manual view, in BOTH toggle directions (`use_time_preserve`).** Because the aligned list changes length/shape (gap-insertion and/or extended-hours filtering), the stale INDEX xlim points at the wrong bars. The gate sets `_preserve_xlim_by_time_on_render = True` and CLEARS `_preserve_xlim_on_render` (they are mutually exclusive — `_compute_slot_window` skips the time-remap while index-preserve is active). `_render` captures the OLD panel candle dates + xlim before `figure.clear()` and `core.viewport.remap_window_by_time` maps the window's calendar edges onto the new series' indices → the framed window stays put regardless of the index shift. Index-preserve is re-armed AFTER the render. `_current_visible_window_ts()` supplies `(lo_ts, hi_ts, is_historical)` from the panel's non-gap candles (uses the FULL `_primary`, not the virtualized slot slice, to decide `is_historical`). A formal double-click drilldown is always an explicit calendar-window selection, even when its right edge equals the latest primary bar; this covers Compare-on → drill the compare chart to today → Compare-off, where removing alignment gaps would otherwise reinterpret the stale aligned indices against a much shorter primary-only series and jump weeks left. A plain manual pan/zoom (`_drilldown_day` is None) still requires `is_historical` plus compare coverage. **The gate does NOT depend on `compare_on`**: toggling compare OFF reverts the primary from the aligned (gap-padded) list to the filtered (ungapped) list, which shifts indices the OTHER way; index-preserve there kept the widened index window on ungapped data → the view drifted AND widened, and that drifted window became the NEXT toggle-on's `keep_window` → compounding "candles creep in from the left on every toggle" (audit `compare-toggle-drilldown-preserve`; pinned by `check_d88` for historical + newest-session drilldown paths and `check_d89` for the manual-pan path). Time-preserving both directions keeps the framed window put regardless.
2. **Keep the viewed bars through the align (`keep_window`) — engaged only for the `compare_lacks` subset.** When the compare cache genuinely doesn't reach the drilled window, the on-screen window is passed as `keep_window` into `apply_pair_filter` → `align_pair`, which retains the viewed primary bars even though they predate the compare's first cached bar (compare gap-filled there). See `core/pairing.spec.md`.
3. **Background-fill the compare, never moving the primary — `compare_lacks` subset.** `_maybe_fill_compare_for_window(lo_ts, hi_ts)` (range-capable intraday providers only — `source_supports_range`) reuses the drilldown's `_targeted_range_fetch` on `_fetch_executor`, coalesced per `(src, cmp, interval, day)` via `_compare_fill_inflight` + token-gated via `_compare_fetch_token`, marshalled back through `_await_future_on_tk`. On completion `_on_compare_fill_done` reloads the compare from disk into `_full_cache`, then re-runs the view-safe toggle so the newly-arrived bars fill the panel with the view held. Emits typed status (`Fetching … (primary view held)` / `loaded` / `No … bars found; primary view kept`).
...
The right-side notebook (watchlist / scanner / sandbox / entries / exits) width is a saved setting `layout.notebook_width_px` that round-trips through File → Save/Load Configuration (settings are in-memory-only; there is no auto-persist, so this is the explicit persistence path the user asked for). Flow:
...
| `ratio_rebase` | set Tk var + trailing `_render()` | ratio rebase-to-100 (see "Ratio render mode") |
...
### Ratio render mode (audit `ratio-render-modes`)
A **ratio** primary/compare symbol (`is_ratio_symbol`, e.g. `AMD/NVDA` — see `data/ratio_source.spec.md`) is typed directly into the ticker box; there is no dialog. It renders like a normal symbol with two differences; non-ratio charts are byte-for-byte unaffected:
- **Always candlesticks.** Ratios draw the same OHLC candlesticks as any symbol (the per-bar quotient's open/high/low/close). There is no line mode.
- **Volume pane hidden for QUOTIENT ratios only** (`_slot_hides_volume` ⇔ `is_quotient_ratio(slot_symbol)`): a quotient has no meaningful volume. In `_render` the slot's volume `height_ratio` is collapsed to ~0 and its `ax_v` is `set_visible(False)` (the axis still exists so every `ax_v` consumer keeps working); the bottom x-tick-label axis skips the hidden volume axis; `_draw_slice` skips `draw_volume` and leaves `vol_bars=None` (so the live-tick fast path safely bails to a full render — ratios don't stream meaningfully). This is part of `_compute_topology_key` (`hide_vol_sig`) so switching normal↔ratio forces a full rebuild.

**Scaled symbols (`SYM/<number>`, e.g. `^VIX/15.87`) are deliberately NOT treated as quotients** for either behaviour below. A scaled symbol is one real instrument on a rescaled axis, so:
- Its **volume pane stays visible** with the underlying's real volume — every volume-weighted study over it remains valid (VWAP scales by the same constant, RVOL is unchanged). Gating on `is_ratio_symbol` (i.e. on the mere presence of a `/`) would throw that away.
- Its **corporate events resolve to the underlying** — see `gui/events_app.spec.md`.
- It **never rebases** (below).

One persisted toggle (View → **Ratio charts (A/B)** → **Rebase to 100**), default **off**:
- **`ratio_rebase`**: `_maybe_rebase_candles` returns a NEW per-slot `Candle` list rebased to **100** (`self._primary`/`_compare` stay raw). **Applies to quotient ratios only.** Rebasing multiplies every bar by `100 / anchor_close`, so for a series already divided by a constant `k` that constant cancels exactly — `(VIXᵢ/k) · 100k/VIX₀ == 100·VIXᵢ/VIX₀` — i.e. rebasing `^VIX/15.87` yields a chart identical to rebasing raw `^VIX`, silently discarding the implied-move units that were the entire point of the divisor. Numerically harmless, semantically destructive, so scaled symbols are excluded at all three gates (`_maybe_rebase_candles`, `_apply_dynamic_ratio_rebase`, and the `_apply_price_scale` tick formatter). Applied at all three candle re-point sites (slow-path slot loop, topology-preserving fast path, `_rewire_slot_candles`) so glyphs + `_autoscale_slot_y` + hover (`_ax_candle_map` / `display_candles`) all read the rebased copy coherently. That bake anchors at **bar 0** (xlim is unknown at candle-build time); **`_apply_dynamic_ratio_rebase` then re-anchors the 100-index to the leftmost VISIBLE bar** and keeps it there on every NON-pan view change. It runs at the tail of `_finalize_render` (both render paths, after xlim is set) and at the top of `_autoscale_y_to_visible` (the universal view-change choke point: pan-end, both zooms, drilldown, view toggles), re-scaling via the pure `_rebased_to_anchor(candles, idx)` helper, re-pointing `_ax_candle_map`, then `_draw_slice` + `_autoscale_slot_y`. Skipped mid pan-drag (`_pan_state` active — re-bakes once on release to realign hover/crosshair); no-op when the left edge already reads ~100. **Live pan tracking** is handled separately by a y-axis tick **formatter**: `_apply_price_scale` installs a `FuncFormatter` on ratio+rebase price axes that multiplies each tick by `_ratio_rebase_y_scale(ps, ax)` = `100 / leftmost_visible_close`, recomputed at format time. Because `interaction._pan_setup_blit` marks `ax.yaxis` an animated artist (redrawn every blit frame), the left edge reads 100 **live as the user pans, with no snap on mouse release**; the formatter is a no-op (scale ≈ 1) at rest where the data is already baked to 100. The divisor cancels the bake anchor, so it is correct whatever units the candle data carries.

Log-scale reuses the existing global `log_price` toggle. The rebase toggle persists via `settings.set` on flip + restore through `_apply_persisted_view_settings` (registered in `tests/_config_roundtrip_spec.py`; pinned by `check_d35b`). Helpers: `_active_symbol_for_slot`, `_slot_hides_volume`, `_maybe_rebase_candles`, `_ratio_failure_message`; handler `_on_menu_toggle_ratio_rebase`. Unit-tested in `tests/unit/gui/test_ratio_render_modes.py`. User guide: `docs/RATIO_CHARTS.md`.

### Indicators decoupled from configuration (audit `config-indicators-decoupled`)
Configuration files (File → Save / Load Configuration) are a layout / theme / view snapshot and are **fully decoupled** from the `IndicatorManager`: the save path does NOT capture indicator state and the load path does NOT mutate it. `_capture_layout_into_settings` captures only notebook width + theme; there is no `settings["indicators"]` key. `apply_loaded_config` never calls `_indicator_manager.load_dict(...)` — a *legacy* config that still carries an `indicators` key is ignored, so loading it can never wipe the user's durable preset library (the "Load Configuration wiped my presets" bug). The **active indicator list is session-only** (clean chart each launch — it was never restored on launch anyway); named **presets** persist independently via `indicators.preset_store` (see below). Pinned by `tests/unit/gui/test_config_indicator_decoupling.py` + `tests/smoke/test_smoke_full.py::check_d35c_config_indicator_decoupling`.

### Indicator presets auto-persist across restarts (audit `indicator-preset-autopersist`)
The named presets specifically (NOT the active-indicator list) ALSO auto-persist to their own file so *Indicators → Save Preset…* survives an app restart without an explicit File → Save Configuration. The in-memory `settings` store can't do this — it never auto-writes and is never read on launch. Instead `__init__` (a) restores presets from `indicators.preset_store.load_presets()` via `_indicator_manager.install_presets(...)` (which fires no event), then (b) subscribes a dedicated `_on_indicator_preset_persist` handler. That handler writes `indicators.preset_store.save_presets(mgr.presets_to_dict(), mgr.active_preset())` on `preset_saved` / `preset_deleted` / `preset_loaded` (preset-table mutations) and on `loaded` (the Manage Indicators dialog's cancel/revert swaps the whole manager state via `load_dict`; File → Load Configuration is decoupled from indicators and never fires it), and ignores render-only events. Kept in a **dedicated** file (`%LOCALAPPDATA%/TradingLab/indicator_presets.json`) so auto-persist never touches the `settings` dirty flag (prompt-on-quit / title indicator stay accurate) and never persists the live active-indicator list. **The bundled starter-pack presets (`data/indicator_presets/preset-*.json`, e.g. "Daily Levels") are seeded into this envelope on first run / upgrade by `templates.seed` — translated from their compact `{id,kind,panel,params}` schema to canonical configs via `preset_store.read_bundled_preset`; `_seed_templates_idle` then calls `_reload_indicator_presets_from_disk()` so they appear in Indicators → Load Preset the same launch (audit `indicator-preset-seeding`).** Pinned by `tests/unit/indicators/test_preset_store.py` + `tests/smoke/test_smoke_full.py::check_d55b_indicator_preset_autopersist`.
...
**Cache-pin set (`_view_pin_tickers` / `compare-ticker-cache-pin`).** `_trim_full_cache` and `_stash_full_cache` protect the currently-viewed tickers from LRU eviction via `_view_pin_tickers()` → the primary ticker PLUS the compare ticker when compare is on (was primary-only before). The bounded `_full_cache` (`full_cache_size`, default 16) churns hard under Alpaca intraday + whole-universe downloads; pinning only the primary let the compare ticker's warm intraday bars get evicted, so a compare toggle then re-fetched from the provider. Pinning both keeps the on-screen pair warm.
