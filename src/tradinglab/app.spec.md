...
`_drilldown_day` records the calendar date on success and survives ticker changes. `_do_scheduled_reload`, `_on_watchlist_double`, `_on_chartstack_promote` route through `_reload_preserving_drilldown(load_fn)` when `_drilldown_day` is set AND `interval == "5m"`. Falls back to most-recent non-gap day if the new ticker has no bars on the exact day; abandons drill only when the new series has no real bars. **`_on_compare_toggle` preserves the drilled-in day (`compare-toggle-drilldown-preserve`):** enabling/disabling compare while drilled into an OLD 5m day (e.g. 2021-12-16) used to snap the chart to the recent right edge — the compare's in-memory 5m cache only holds the recent trailing window (deep-history providers cap intraday depth — Alpaca 5m ≈ `_DEEP_HISTORY_INTRADAY_DAYS["5m"]` = 120 days), so `core.pairing.align_pair`'s low-end intersection (`lo_day = max(p[0], c[0])`) DROPPED the drilled day from BOTH series, invalidating the preserved index xlim. Fix: when drilled-in (`_drilldown_day` set AND intraday) and toggling compare ON, first pull the compare's FULL `disk_cache.load` history into `_full_cache` (universe download / prior drills populate it — mirrors `_on_drilldown_fetch_done`'s disk-reload). Because the drilled day then survives the align AND a same-grid intraday pair inserts no gap bars, the primary's bar indices are unchanged and the UNCHANGED toggle body's `preserve`-xlim render already holds the drilled-day window — no explicit re-zoom (an explicit `_zoom_primary_to_date` recompute raced in-flight companion prefetches → primary/compare length desync). The toggle body is extracted into `_apply_compare_toggle(compare_on)` (behaviour-identical) so the drilldown wrapper can disk-preload then run it verbatim. Pinned by `check_d86`.
