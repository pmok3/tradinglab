...
1. **Preserve the view by TIME, not index — engaged for EVERY historical intraday view toggle, in BOTH directions (`use_time_preserve`).** Because the aligned list changes length/shape (gap-insertion and/or extended-hours filtering), the stale INDEX xlim points at the wrong bars. The gate sets `_preserve_xlim_by_time_on_render = True` and CLEARS `_preserve_xlim_on_render` (they are mutually exclusive — `_compute_slot_window` skips the time-remap while index-preserve is active). `_render` captures the OLD panel candle dates + xlim before `figure.clear()` and `core.viewport.remap_window_by_time` maps the window's calendar edges onto the new series' indices → the framed window stays put regardless of the index shift. Index-preserve is re-armed AFTER the render. `_current_visible_window_ts()` supplies `(lo_ts, hi_ts, is_historical)` from the panel's non-gap candles (uses the FULL `_primary`, not the virtualized slot slice, to decide `is_historical`) and works for BOTH a formal double-click drilldown AND a plain manual pan/zoom back into history (`_drilldown_day` may be None). **The gate does NOT depend on `compare_on`**: toggling compare OFF reverts the primary from the aligned (gap-padded) list to the filtered (ungapped) list, which shifts indices the OTHER way; index-preserve there kept the widened index window on ungapped data → the view drifted AND widened, and that drifted window became the NEXT toggle-on's `keep_window` → compounding "candles creep in from the left on every toggle" (audit `compare-toggle-drilldown-preserve`; pinned by `check_d88` for the drilldown path and `check_d89` for the manual-pan path). Time-preserving both directions keeps the framed window put regardless.
...
**Gates, scoped to historical intraday views only (so the right edge / today keeps the fast index-preserve):** `use_time_preserve` = `intraday` (interval is intraday) AND `is_historical` (`_current_visible_window_ts()[2]` — the visible right edge sits before the primary's last real bar) — engaged for BOTH toggle directions (on AND off) and independent of `_drilldown_day`. This is the OUTER gate — it engages TIME-preserve for both failure modes AND for the manual-pan-into-history case (the still-reproducing half of the creep bug: a user reaches a 5m historical window by panning/zooming, not a formal drilldown, so `_drilldown_day` is None). The INNER gate `compare_lacks` = `use_time_preserve` AND `compare_on` AND `not _compare_cache_covers(lo_ts)` (the compare cache does NOT reach at/before the window's left edge — **including a genuinely EMPTY cache**, e.g. SPY preloaded at 1d only, so its 5m was never fetched) additionally engages `keep_window` + background-fill (only meaningful when turning compare ON — you can't fill compare while turning it off). **Empty-cache targeted-first-load (`compare-toggle-targeted-first-load`):** when `compare_lacks` is set and the cache miss reaches `_apply_compare_toggle`'s no-in-memory-data fallback, a range-capable provider (`source_supports_range(src)`) on an intraday interval renders the primary IMMEDIATELY (holding the drilled window via `keep_window`, compare empty) and returns — instead of the old FULL `_load_data` of the compare's entire recent history (~120 days of 5m ≈ 5s on Alpaca). The caller's `_maybe_fill_compare_for_window(lo_ts, hi_ts)` then fetches ONLY the viewed day (~1 API page) and re-renders with the compare when it lands. Non-range / right-edge cache misses keep the `_load_data` fallback. A right-edge / today view (`is_historical` False) keeps the existing index-preserve; `check_d52` frames an interior window and toggles compare once — both preserve strategies satisfy its coarse "framed window preserved" assertion, and repeated-toggle no-creep is pinned separately by `check_d88`/`check_d89`. The toggle body is extracted into `_apply_compare_toggle(compare_on, *, keep_window=None)` (behaviour-identical) so both the gated path and `_on_compare_fill_done` can invoke it. Instance state: `_compare_fill_inflight: set`, `_compare_fetch_token: int` (init in `__init__`). **The cache-hit fast path's `_ensure_compare_prefetched(force=True)` (a "catch new ticks" background refresh) is skipped when the view is HISTORICAL** (`_current_visible_window_ts()[2]`): a drilled/panned PAST window can't gain new ticks, so force-refetching a range-capable provider (Alpaca) on every compare toggle was needless and slow — a genuinely missing window is filled by `_maybe_fill_compare_for_window` instead (audit `compare-toggle-historical-no-refetch`). Pinned by `check_d86` (deep-history no-jump + fill-on-completion), `check_d87` (in-coverage grid-mismatch: sparse primary + dense compare inserts gaps → time-preserve holds the drilled day as the left edge; fails without the broadened `use_time_preserve` gate), `check_d88` (REPEATED drilldown toggle on/off must not creep the window), `check_d89` (REPEATED manual-pan toggle on/off must not creep the window — the `_drilldown_day`-independent path), `check_d92` (compare toggle on a drilled day shows EXACTLY that day — no next-day bar from the half-integer-xlim remap round bug — and does not force-refetch on a historical view), and `check_d93` (empty compare 5m cache on a range-capable provider does a TARGETED single-day fill, not a full `_load_data`).

Cleared by `_reset_view` and `_on_explicit_axis_change`. On an **interval** change the drilldown day + both preserve flags drop (right-edge snap). On a **source-only** change (same ticker + interval, different provider) the drilldown day still clears but the view is preserved by TIME (`_preserve_xlim_by_time_on_render=True`) so a different-length provider series doesn't reinterpret the stale bar-index window as another calendar day (the "switch source → jump a month back" bug; audit `source-switch-view-preserve`). Source vs interval is classified via `_prev_axis_source` / `_prev_axis_interval` (seeded in `__init__`, updated each call). A live `_next_bar_fetch_tick` could re-arm index-preservation while the async switch load is in flight, so `_on_explicit_axis_change` raises `_axis_switch_inflight=True` before `_load_data_async()`; the poll tick bails while it's set and `_load_data` lowers it (top of method) so polling re-arms via `_schedule_next_bar_fetch`. **Durable time-preserve across the async boundary (`source-switch-view-preserve`):** the poll-tick guard is NOT sufficient by itself — an intervening render from a *non-poll* path (a Compare-ON `_ensure_compare_prefetched(force=True)` prefetch landing, a `_refresh_daily_synth_for_active_view`, a reference-data redraw) can fire during the async switch load and (a) CONSUME the one-shot `_preserve_xlim_by_time_on_render` and/or (b) re-arm `_preserve_xlim_on_render`. Index-preserve WINS over the time-remap in `_compute_slot_window` (the remap is skipped while index-preserve is active), so the switch's completing render then reused the stale ~200-bar recent bar-INDEX window against the new provider's ~2×-longer series → **the view jumped years back** (the user's "Compare ON→OFF then switch yfinance→alpaca → chart moved to 2021" repro). Fix: `_on_explicit_axis_change` sets a DURABLE `_pending_axis_switch_time_preserve = source_only_change` (companion to the one-shot flag), and the switch's completing `_load_data` (detected via `was_axis_switch` = `_axis_switch_inflight` captured before it's lowered) RE-ASSERTS `_preserve_xlim_by_time_on_render=True` + `_preserve_xlim_on_render=False`, clears the pending flag, and renders SYNCHRONOUSLY (so no further event can consume the re-asserted flags before the render). Interval changes leave the pending flag False (right-edge snap unaffected); sandbox `_load_data` early-returns before the re-assertion. Pinned by `check_d94` (simulates the mid-switch index-preserve re-arm and asserts the view stays recent, not years-back). Pre/Post has its own handler `_on_prepost_toggle` (render-scope, not view-scope) that drills via `_reload_preserving_drilldown` and re-zooms to fit the new bar count.
