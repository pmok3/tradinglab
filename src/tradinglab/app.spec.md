...
Index symbols are spelled differently by every vendor (`^VIX` yfinance / `$VIX` Schwab / `I:VIX` Polygon — see `data/index_aliases.spec.md`). `_reresolve_symbols_for_source` rewrites `ticker_var` and `compare_ticker_var` into the ACTIVE source's vocabulary and is called at the top of both `_load_data` and `_load_data_async` (after the sandbox early-return, so sandbox symbol handling is untouched). Because `index_aliases.resolve_symbol` canonicalises its input first and is idempotent, that one seam serves both entry points: typing `VIX` rewrites the box to `^VIX`, and switching source re-resolves `^VIX` → `$VIX` (the axis-change handler ends in a reload). No separate source-change hook exists, so there is no second copy of the rule to drift. Ratio legs resolve independently and a scale constant is preserved (`VIX/15.87` → `^VIX/15.87`). Best-effort: never raises into a load path. Unit-tested in `tests/unit/gui/test_source_change_reresolve.py`.
