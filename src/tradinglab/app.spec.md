...
1. **Preserve the view by TIME, not index — engaged for EVERY historical-day drilldown toggle, in BOTH directions (`use_time_preserve`).** Because the aligned list changes length/shape (gap-insertion and/or extended-hours filtering), the stale drilled INDEX xlim points at the wrong bars. The gate sets `_preserve_xlim_by_time_on_render = True` and CLEARS `_preserve_xlim_on_render` (they are mutually exclusive — `_compute_slot_window` skips the time-remap while index-preserve is active). `_render` captures the OLD panel candle dates + xlim before `figure.clear()` and `core.viewport.remap_window_by_time` maps the window's calendar edges onto the new series' indices → the drilled day stays framed regardless of the index shift. Index-preserve is re-armed AFTER the render. `_current_visible_window_ts()` supplies `(lo_ts, hi_ts, is_historical)` from the panel's non-gap candles (uses the FULL `_primary`, not the virtualized slot slice, to decide `is_historical`). **The gate does NOT depend on `compare_on`**: toggling compare OFF reverts the primary from the aligned (gap-padded) list to the filtered (ungapped) list, which shifts indices the OTHER way; index-preserve there kept the widened index window on ungapped data → the view drifted AND widened, and that drifted window became the NEXT toggle-on's `keep_window` → compounding "candles creep in from the left on every toggle" (audit `compare-toggle-drilldown-preserve`; pinned by `check_d88`). Time-preserving both directions keeps the drilled day framed regardless.
...
**Gates, scoped narrowly to avoid regressing manual interior zoom:** `use_time_preserve` = `in_drilldown` (`_drilldown_day` set AND intraday) AND `is_historical` (visible right edge sits before the primary's last real bar) — engaged for BOTH toggle directions (on AND off). This is the OUTER gate — it engages the TIME-preserve for both failure modes. The INNER gate `compare_lacks` = `use_time_preserve` AND `compare_on` AND `_compare_cache_earliest_gt(lo_ts)` (compare HAS cached data starting AFTER the window — an EMPTY cache returns False, so a fresh compare uses the ordinary cache-miss load path) additionally engages `keep_window` + background-fill (only meaningful when turning compare ON — you can't fill compare while turning it off). The drilldown scope is deliberate: a normal manual rubber-band zoom keeps the existing index-preserve (`check_d52` clears its inherited `_drilldown_day` to stay on that ordinary path). The manual-pan-into-history case is a deferred follow-up. The toggle body is extracted into `_apply_compare_toggle(compare_on, *, keep_window=None)` (behaviour-identical) so both the gated path and `_on_compare_fill_done` can invoke it. Instance state: `_compare_fill_inflight: set`, `_compare_fetch_token: int` (init in `__init__`). Pinned by `check_d86` (deep-history no-jump + fill-on-completion), `check_d87` (in-coverage grid-mismatch: sparse primary + dense compare inserts gaps → time-preserve holds the drilled day as the left edge; fails without the broadened `use_time_preserve` gate), and `check_d88` (REPEATED toggle on/off must not creep the window — the OFF direction time-preserve).
