...
**`_on_compare_toggle` is view-safe — the primary NEVER jumps (`compare-toggle-drilldown-preserve`).** Enabling/disabling compare while drilled into an OLD 5m day (e.g. 2021-12-16) used to snap the chart to the recent right edge: the compare's cached 5m history only holds the recent trailing window (deep-history providers cap intraday depth — Alpaca 5m ≈ `_DEEP_HISTORY_INTRADAY_DAYS["5m"]` = 120 days), so `core.pairing.align_pair`'s low-end intersection (`lo_day = max(p0, c0)`) DROPPED the drilled day from BOTH series, invalidating the preserved index xlim → "jumped to June 5th". The prior fix (disk-preload the compare's full history into `_full_cache` before the toggle) could not work when disk itself lacks the old compare data (the recent Alpaca trailing window). Replaced by a two-layer, no-jump design:

1. **Keep the viewed bars through the align (`keep_window`).** `_current_visible_window_ts()` reads the on-screen primary xlim → `(lo_ts, hi_ts, is_historical)` from the panel's non-gap candles (uses the FULL `_primary` — not the virtualized slot slice — to decide `is_historical`). That window is passed as `keep_window` into `apply_pair_filter` → `align_pair`, which retains the viewed primary bars even though they predate the compare's first cached bar (compare gap-filled there). See `core/pairing.spec.md`.
2. **Preserve the view by TIME, not index.** Because the aligned list changes length/shape, the stale drilled INDEX xlim would overflow → right-edge snap. So the gate sets `_preserve_xlim_by_time_on_render = True` and CLEARS `_preserve_xlim_on_render` (the index-preserve): `_compute_slot_window` skips the time-remap while index-preserve is active, so both cannot be on together. Index-preserve is re-armed AFTER the render (the remapped index xlim is correct again).
3. **Background-fill the compare, never moving the primary.** `_maybe_fill_compare_for_window(lo_ts, hi_ts)` (range-capable intraday providers only — `source_supports_range`) reuses the drilldown's `_targeted_range_fetch` on `_fetch_executor`, coalesced per `(src, cmp, interval, day)` via `_compare_fill_inflight` + token-gated via `_compare_fetch_token`, marshalled back through `_await_future_on_tk`. On completion `_on_compare_fill_done` reloads the compare from disk into `_full_cache`, then re-runs the view-safe toggle so the newly-arrived bars fill the panel with the view held. Emits typed status (`Fetching … (primary view held)` / `loaded` / `No … bars found; primary view kept`).

**Gate (`compare_lacks`), scoped narrowly to avoid regressing manual interior zoom:** `in_drilldown` (`_drilldown_day` set AND intraday) AND `is_historical` AND `compare_on` AND `_compare_cache_earliest_gt(lo_ts)` (compare HAS cached data starting AFTER the window — an EMPTY cache returns False, so a fresh compare uses the ordinary cache-miss load path). The drilldown scope is deliberate: a normal manual rubber-band zoom keeps the existing index-preserve (which frames the window tighter — the time-remap in `core.viewport.remap_window_by_time` snaps to the widest enclosing bars; `check_d52` pins the tight framing, and clears its inherited `_drilldown_day` to stay on the ordinary path). The manual-pan-into-history case is a deferred follow-up. The toggle body is extracted into `_apply_compare_toggle(compare_on, *, keep_window=None)` (behaviour-identical) so both the gated path and `_on_compare_fill_done` can invoke it. Instance state: `_compare_fill_inflight: set`, `_compare_fetch_token: int` (init in `__init__`). Pinned by `check_d86` (no-jump on toggle + fill-on-completion).
