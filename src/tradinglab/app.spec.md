...
- **Compare slot mirrors the primary's window (shared x-axis).** Every slot's price/volume/indicator axes are created with `sharex=ax_p1`, so a `set_xlim` on ANY of them moves ALL. The ticker-switch time-remap is **primary-only** (it maps the *previous primary's* visible calendar window onto the new symbol). Both render loops therefore capture the primary slot's resolved `(lo, hi)` and pass it as `primary_window` when computing the **compare** slot; `_compute_slot_window` returns that window verbatim for any non-primary slot (clamped to the slot's `n`, xlim applied). Without this, the compare slot fell through to the right-edge default and its `set_xlim` **clobbered the primary's remapped window on the shared axis** — the compare-mode ticker-switch bug where the chart reset to the prior session (the user had to pan back to today's price action). No-op for single-chart mode; identical result to the old code in the default/right-edge case (both slots resolve the same window anyway).
