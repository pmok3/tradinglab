...
- **`ratio_rebase`**: `_maybe_rebase_candles` returns a NEW per-slot `Candle` list rebased to **100 at the first loaded bar** (`self._primary`/`_compare` stay raw). Applied at all three candle re-point sites (slow-path slot loop, topology-preserving fast path, `_rewire_slot_candles`) so glyphs + `_autoscale_slot_y` + hover (`_ax_candle_map` / `display_candles`) all read the rebased copy coherently. Anchor is fixed (first bar, not visible-edge) so pan/zoom don't re-anchor.
